...
async def check(call: types.callback_query):
    ...
    proverka1 = (await bot.get_chat_member(chat_id='@qwmoneyrewiews', user_id=call.message.chat.id)).status
...
async def walled(call: types.callback_query):
    ...
    bat_d = types.InlineKeyboardButton(text='⭐ QIWI с балансом 157 руб | 50 руб', callback_data='qiwi_4')
...
async def qiwi_walled(call: types.callback_query):
    ...
    if qiwi_number == 1: #Первый киви кошелек
        ...
        await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                    text='Вы выбрали :\n'
                                         '<b>⭐ QIWI с балансом 157 руб | 50 руб</b>\n\n'
                                         '🔸 Qiwi кошелёк, цена: 50₽.\n'
                                         '💰 Баланс кошелька: 157 руб\n\n'
                                         '▪При покупки кошелька, вы получаете: логин, пароль\n\n'
                                         '☑️Смс код выключен!\n\n'
                                         '💠 Цена: 50 рублей\n'
...
async def bue_qiwi(call: types.callback_query):
    ...
    if number == 1:
        ...
        await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                    text='Вы выбрали:\n'
                                         '⭐ QIWI с балансом 157 руб | 50 руб\n\n'
                                         '💠 Цена: 50 рублей\n'
                                         'Для подтверждения покупки, нажимай кнопку👇', reply_markup=markup)
